...
from Data.rao_data import theta_e_data, theta_n_data
import json
from pathlib import Path

# --- 1. GET PRIMARY ENGINE INPUTS ---
from EngineParameters import(fuel, oxidizer, Pc, Pe, T_target, MR, Cstar_eff, Cf_eff)

# Initialize Propellants
rocket = CEA_Obj(oxName=oxidizer, fuelName=fuel)
...
def runContourCode():
    ...
    csv_filename = geometry_dir / "current_contour.csv"
    ...
    # --- 6. EXPORT PARAMETERS AS JSON ---

    parameters = {
        "R_t": float(R_t),
        "eps": float(eps),
        "eps_c": float(eps_c_opt),
        "L_ch": float(Lc_opt),
        "theta_c": float(theta_c_opt),
        "Rtc_Rt": float(Rtc_opt),
        "Rtd_Rt": float(Rtd_Rt),
        "Lf": float(Lf_opt),
        "theta_n": float(theta_n_deg),
        "theta_e": float(theta_e_deg)
    }

    with open(geometry_dir / "current_parameters.json", "w") as file:
